...
`)
	...
	github := resolved[0]
	if github.Identity != "github/issue-intake" || github.Every != 5*time.Minute || github.Label != "machinist:requested" || github.GitHubRepositories["machinist"] != "mitkox/esf" {
	...
	interval := resolved[1]
	if interval.Identity != "interval/repository-audit" || interval.Repository != "machinist" || interval.GitHubRepository != "mitkox/esf" {
